      ...
      <div class="footer">
        ...
        <div v-if="!isMyCouponInfo">
          ...
          <div v-if="!coupon.receive">
            ...
            <button v-if="coupon.num===0" @click.stop="noNumTips()">已领完</button>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        params: {
          'couponId': parseInt(this.$route.params.couponId),
          'couponActivityId': parseInt(this.$route.params.couponActivityId)
        },
        userParams: {
    ...
    methods: {
      async getCouponDetail() { // 获取优惠劵详情
        this.params.couponId = parseInt(this.$route.params.couponId)
        this.params.couponActivityId = parseInt(this.$route.params.couponActivityId)
        let data = await couponService.getActivityCouponDetail(this.params, {showLoading: true})
        if (data.length === 0) {
          console.log('没有数据')
        }
        data.data['receive'] = false
        data.data.canReceive = false
        data.data.receiveTime = data.data.robTime || null
        data.data.startInterval = false
        data.data.intervalMinute = '00'
        data.data.intervalSecond = '00'
      ...
      async receiveCoupon() {
        ...
        let data = await couponService.receiveCoupon({
          ...
          type: 1,
          couponActivityId: this.$route.params.couponActivityId
        }).catch(e => {
          setTimeout(() => {
      ...
      noNumTips() {
        Util.alert('优惠券已经被领取完了，去看看别的优惠吧')
      }
    }
  }
</script>
