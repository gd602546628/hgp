    ...
    methods: {
        ...
        getAmendComment.getOrderInfo(params).then(data => {
          data.data[0].prdInfos.forEach(item => {
            if (item.priceId === this.$route.params.priceId){
              this.prdInfo = item
              if (item.storeOrderComment.commentImg){
                this.answer = JSON.parse(item.storeOrderComment.commentImg)
      ...
      submit(){ // 提交评价
        ...
        if (params.content === '') {
          ...
          getAmendComment.getmodifyComment(params).then(async data => {
            if (data.code === '000000') {
              await Util.alert({
                message: '修改成功！'
              })
