  ...
  <zyt-page :on-infinite="loadMore"
    ...
    <div class="online-my-order" slot="main">
      <div class="tab">
        ...
        <div class="tab-item " @click.stop="tabClick(2)" :class="{active:params.status==2}">待消费</div>
...
<script>
  ...
  export default{
    ...
    methods: {
      tabClick(status){
        this.$router.replace({name: 'offlineMyOrder', params: {status: status}})
      },
      async init(){
        this.dataFinish = true
        if (this.$route.name !== 'offlineMyOrder') {
          return
        }
        this.orderList = []
      ...
      goLock(){
        this.$router.push({name: 'selectedGoods'})
      }
    }
  }
</script>
